...
export default function EsewaVerifyContent() {
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      ...
      // Early exit if required params are missing
      if (!data || !paymentId) {
        setStatus("failed");
        return;
      }
      ...
      try {
        const res = await paymentAPI.verifyEsewa(data, paymentId);
      ...
      <div className="bg-white rounded-2xl border border-[#E5E9EB] shadow-lg p-8 max-w-md w-full text-center flex flex-col items-center gap-4">
        ...
        {status === "success" && (
          <>
            ...
            <p className="text-sm text-[#9AA6AC]">
              Your eSewa payment has been verified and recorded.
            </p>
            {transactionId && (
        ...
        {status === "failed" && (
          <>
            ...
            <p className="text-sm text-[#9AA6AC]">
              Your eSewa payment could not be verified. Please try again.
            </p>
            <button
              onClick={() => router.push("/manufacturer/order-management")}
              className="w-full h-10 rounded-xl bg-teal-500 hover:bg-teal-600 text-white text-sm font-medium transition"
